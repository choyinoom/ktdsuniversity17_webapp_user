					...
					<script type="text/javascript">
						...
						function fn_getPage(pageNo) {	// 페이지 목록 갱신
							fn_sendRequest(pageNo)
								.then((response) => fn_drawingTable(response, pageNo))
						...
						function fn_getNoticesList() { // 검색
							...
							fn_sendRequest(pageNo)
								.then((response) => {
									fn_drawingTable(response, pageNo);
									fn_paginationCtrl(response, pagingSize);
								})
								.catch((error) => { console.log(error) });
						}

						function fn_drawingTable(response, pageNo) { // 새로운 데이터로 테이블 새로 그리기
							const articlesList = response.articlesList;
							const noticesCnt = response.noticesCnt;
							var index = Math.max(articlesList.length, noticesCnt - 10 * (pageNo - 1));
